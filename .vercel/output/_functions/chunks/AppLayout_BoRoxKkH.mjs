import { e as createAstro, f as createComponent, m as maybeRenderHead, h as addAttribute, r as renderTemplate, l as renderHead, k as renderComponent, n as renderSlot } from './astro/server_C6rdUW15.mjs';
import 'piccolore';
/* empty css                            */
import 'clsx';
...
const $$AppLayout = createComponent(($$result, $$props, $$slots) => {
  ...
  const inferredActiveNav = activeNav ?? navItems.find((item) => currentPath === item.href || currentPath.startsWith(`${item.href}/`))?.id;
  return renderTemplate`<html lang="es-MX"> <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover"><meta name="theme-color" content="#f9f9f9"><title>${title}</title><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet"><link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@24,500,0,0" rel="stylesheet">${renderHead()}</head> <body${addAttribute(`min-h-screen ${bodyClass}`, "class")}> <div class="mx-auto flex min-h-[100dvh] w-full max-w-[430px] flex-col bg-[#f9f9f9]"> ${topBar && renderTemplate`${renderComponent($$result, "TopAppBar", $$TopAppBar, { ...topBar })}`} <main${addAttribute(`flex-1 px-4 ${navItems.length > 0 ? "pb-24" : "pb-6"} ${pageTitle ? "pt-6" : "pt-4"}`, "class")}> ${pageTitle && renderTemplate`<h1 class="mb-6 text-[2.6rem] font-extrabold leading-tight tracking-[-0.02em] text-[#213138]">${pageTitle}</h1>`} ${renderSlot($$result, $$slots["default"])} </main> ${navItems.length > 0 && renderTemplate`${renderComponent($$result, "BottomNav", $$BottomNav, { "items": navItems, "activeId": inferredActiveNav })}`} </div> </body></html>`;
