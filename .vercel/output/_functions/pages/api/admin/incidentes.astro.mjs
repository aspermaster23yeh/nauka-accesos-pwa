...
const GET = async ({ locals }) => {
  ...
  const { data, error } = await supabase.from("incidentes").select("id, created_at, estado, descripcion, severidad, visitante_nombre, lote_number").eq("complejo_id", locals.profile.complejo_id ?? "complejo-1").order("created_at", { ascending: false }).limit(100);
  if (error) {
